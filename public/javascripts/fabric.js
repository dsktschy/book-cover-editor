...
(function(global) {
  ...
  fabric.Text = fabric.util.createClass(fabric.Object, /** @lends fabric.Text.prototype */ {
    ...
    _renderViaNative: function(ctx) {
      ...
      if( !this.lineWidth ){
        // デフォルト値
        this.lineWidth = ( this.writingMode === 'lr' )
          // 横書き
          ? ctx.measureText( this.text ).width
          // 縦書き
          : ctx.measureText("あ").width * this.text.length;
      }
      // 元の文字列を改行文字毎に分割
      var textLines = this.text.split(this._reNewline);
      // 最大行幅と最大行数を反映
      textLines = this._wrapTextLines( ctx, textLines );
    ...
    // 最大行幅と最大行数を反映した文字列を返す
    _wrapTextLines: function( ctx, textLines ){
      
      var wrappedLines = [],
          _this = this;

      // 最大行幅と最大行数を一行にだけ反映
      function wrapTextLine( ctx, line, lineIndex ){
            // 一行の高さ
        var lineHeight = _this._getHeightOfLine(),
            // 現在行の幅
            lineWidth = ( _this.writingMode === 'lr' )
              // 横書き
              ? ctx.measureText( line ).width
              // 縦書き
              : ctx.measureText("あ").width * line.length,
            // 行を折り返した回数
            count = 0,
            // 一行を折り返した文字列
            wrappedLine = [];
        // 現在行幅が最大行幅を超えている場合
        if( _this.lineWidth < lineWidth ) {
              // 1行を文字毎に配列に格納
          var chars = line.split( '' ),
              // 1文字目
              line = chars[ 0 ];
          // 1行全ての文字に対して（どこで折り返すか）
          for( var c = 1, len = chars.length; c < len; c++ ) {
            // (現在文字+次の文字)の幅
            var w = ( _this.writingMode === 'lr' )
                  ? ctx.measureText( line + chars[ c ] ).width // 横書き
                  : ctx.measureText("あ").width * ( line.length + 1 ); // 縦書き
            // (現在文字+次の文字)の幅が最大行幅を超えた場合
            if( w > _this.lineWidth ) {
              // ここまでを一行とする
              wrappedLine.push( line );
              // 最大行数に達した場合
              if( lineIndex + count + 1 >= _this.lineMax ){
                // 折り返した文字列を返して終了
                return wrappedLine;
              }
              // 折り返し回数インクリメント
      ...
      // 最大行数まで一行毎に
      for (var wl = 0, tl = 0; wl < this.lineMax && tl < textLines.length; wl++, tl++) {
        // 最大行幅と最大行数を反映
        var wrappedLine = wrapTextLine( ctx, textLines[ tl ], wl );
        // 返り値の配列に追加
        Array.prototype.push.apply( wrappedLines, wrappedLine );
        // 折り返した回数分ループの残りを減らす
        wl += ( wrappedLine.length - 1 );
      }
    ...
    _translateForTextAlign: function(ctx) {
      if (this.writingMode !== 'tb' && this.textAlign !== 'left' && this.textAlign !== 'justify') {
        ctx.save();
    ...
    _getTextHeight: function(ctx, textLines) {
      ...
      var charHeight = ctx.measureText("あ").width,
          maxHeight = charHeight * textLines[0].length || ctx.measureText('|').width;
      for (var i = 1, len = textLines.length; i < len; i++) {
        var currentLineWidth = charHeight * textLines[ i ].length;
        if (currentLineWidth > maxHeight) {
          maxHeight = currentLineWidth;
        }
      }
      return maxHeight;
    },
    ...
    _getTextWidth: function(ctx, textLines) {
      ...
      if( this.writingMode === 'tb' ){
        return -( this._getHeightOfLine() * textLines.length );
      }
      // 横書き
    ...
    _renderTextLine: function(method, ctx, line, left, top, lineIndex) {
      // 縦書き
      if( this.writingMode === 'tb' ){
        this._renderTextLineVertical( method, ctx, line, left, top, lineIndex );
        return;
      }

      // lift the line by quarter of fontSize
      // top -= this.fontSize / 4;
      top -= this._getHeightOfLine() * 0.455;
    ...
    _renderTextLineVertical: function( method, ctx, line, orgLeft, left, lineIndex ){
      ...
      for( var i = 0; i < line.length; i++ ){
        ...
        top += charHeight;
        // 半角英数記号を全角に
        var c = line[ i ].replace( /[!-~]/g, function( s ) {
              return String.fromCharCode( s.charCodeAt( 0 ) + 0xFEE0 );
            } );
        // 右上にずらす文字の場合
        if( this.shift.indexOf( c ) >= 0 ) {
          this._renderChars( method, ctx, c, left + ( charHeight / 10 ), top - ( charHeight / 10 ) );
        // 回転が必要な文字の場合
        } else if( this.rotative.indexOf( c ) >= 0 ) {
          // 原点をオブジェクト中央からオブジェクト右上に（widthは負の値）
          ctx.translate( -this.width/2, -this.height/2 );
          // 原点を文字の中央上に
        ...
        // 上記以外
          ...
          // 描画
          this._renderChars(method, ctx, c, left, top);
        }
      }
    },
    ...
    _renderTextFill: function(ctx, textLines) {
      ...
      for (var i = 0, len = textLines.length; i < len; i++) {
        ...
        this._renderTextLine(
          ...
          this._getLeftOffset(),
          ( this.writingMode === 'lr' )
            // 横書き時のy座標: topから行数分下へずらした座標
            ? this._getTopOffset() + lineHeights
            // 縦書き時のx座標: leftから行数分左へずらした座標
            : this._getLeftOffset() - lineHeights,
          i
        );
      }
// // オブジェクトの中心が原点で、右下方向ではなく右上方向に描画される
// ctx.fillText('★', 0, 0);
    },
